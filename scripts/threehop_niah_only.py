#!/usr/bin/env python3
"""3-hop query-aware attention tracing v3 - in-hook chunked (max over groups).

The best version: computes 3-hop scores inside the hook using chunked attention,
with max-over-groups (not mean). This matches the successful v0 but avoids OOM
by not storing the full L×L attention matrix.
"""
...
def compute_threehop_scores(model, tokenizer, context_ids,
                            fanin_temp=1.0, chunk_size=1024):
    """3-hop scoring computed entirely inside the prefill hook.

    Uses max-over-groups (not mean) and chunked processing to avoid OOM.
    Key: does NOT store the full L×L attention matrix.
    """
    ...
    def make_hook(layer_idx):
        def hook_fn(module, args, kwargs, output):
            ...
            with torch.no_grad():
                ...
                # q: [1, n_q_heads, L, d] → group: [1, n_kv_heads, n_groups, L, d]
                ...
                for hi in range(n_kv_heads):
                    ...
                    # ═══ Step 1: OUTGOING from user input (max over groups) ═══
                    ...
                    step1 = user_attn.amax(dim=(0, 1))  # [L]
                    del user_logits, user_attn, user_q

                    # ═══ Pass 1: Compute exact fan-in (column sums of attention) ═══
                    ...
                    for start in range(0, ctx_len, chunk_size):
                        ...
                        fan_in += chunk_attn.sum(dim=0)  # column sum
                    ...
                    # ═══ Pass 2: Step 2+3 with exact fan-in weighting ═══
    ...
    print(f"  Prefill+scoring: {total_time:.1f}s (hook time: {timing['total']:.1f}s)")
...
def analyze_niah_scores(tokenizer, score_val, ctx_ids, target_key, target_value):
    ...
    if char_pos < 0:
        print("  Cannot find target needle")
        return
    cum = ""
    ...
    for li in range(0, score_val.shape[0], 4):
...
def main():
    ...
    print("\n" + "=" * 80)
    print("3-HOP v3: IN-HOOK CHUNKED (max over groups)")
    print("=" * 80)
    ...
    for n_dist in [30, 100]:
        ...
        scores, _ = compute_threehop_scores(model, tokenizer, ctx_ids)

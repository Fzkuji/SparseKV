#!/usr/bin/env python3
"""Power sharpening for adaptive KV eviction.

Correct approach: p_new = p^alpha / sum(p^alpha), alpha > 1.
Each iteration squares probabilities and renormalizes → peaked tokens
get more peaked, flat tokens vanish.

Keep tokens with weight > 1/N after sharpening.
"""
...
def power_sharpen(weights, alpha):
    """Sharpen: p_new = p^alpha / sum(p^alpha).
    alpha > 1 sharpens, alpha < 1 smooths.
    """
    w = weights.copy()
    w = np.maximum(w, 1e-30)  # avoid log(0)
    # Use log-space for numerical stability
    log_w = np.log(w) * alpha
    log_w -= log_w.max()
    w = np.exp(log_w)
    w = w / w.sum()
    return w
...
def iterative_power_sharpen(weights, n_iter, alpha=2.0):
    """Apply power sharpening n_iter times.
    Each iteration: p = p^alpha / sum(p^alpha)
    """
    w = weights.copy()
    for _ in range(n_iter):
        w = power_sharpen(w, alpha)
    return w


def select_tokens_power_sharpen(attn, n_iter, alpha, n_kv_heads, n_groups, n_q, ctx_len,
                                 protect_sink=4, protect_recent=4):
    """For each KV head: sharpen attention, keep tokens above uniform threshold.
    ...
    for hi in range(n_kv_heads):
        ...
        for gi in range(n_groups):
            for qi in range(n_q):
                ...
                sharpened = iterative_power_sharpen(weights, n_iter, alpha)
...
def main():
    ...
    # ═══════════════════════════════════════════
    # Demo: power sharpening effect
    # ═══════════════════════════════════════════
    print(f"\n{'='*100}")
    print("Demo: power sharpening on KV5, group0, q_token0, Layer 24")
    print(f"{'='*100}")

    demo_w = all_attn[24, 5, 0, 0].numpy()
    print(f"  Original: top-5 = {np.sort(demo_w)[::-1][:5]}")
    print(f"  Original: #above uniform = {(demo_w > 1.0/ctx_len).sum()}, entropy = {-(demo_w * np.log(demo_w + 1e-30)).sum():.3f}")

    for config in [(2.0, 1), (2.0, 2), (2.0, 3), (2.0, 5), (3.0, 1), (3.0, 2), (5.0, 1), (10.0, 1)]:
        alpha, n_iter = config
        sharp = iterative_power_sharpen(demo_w, n_iter, alpha)
        above = (sharp > 1.0/ctx_len).sum()
        top5 = np.sort(sharp)[::-1][:5]
        ndl_above = sum(1 for p in needle_pos if sharp[p] > 1.0/ctx_len)
        entropy = -(sharp * np.log(sharp + 1e-30)).sum()
        print(f"  α={alpha:.0f}, iter={n_iter}: top-5 = [{', '.join(f'{v:.4f}' for v in top5)}], "
              f"#above = {above}, ndl = {ndl_above}/{len(needle_pos)}, H = {entropy:.3f}")

    # ═══════════════════════════════════════════
    # Also demo on a NON-retrieval head (KV0)
    # ═══════════════════════════════════════════
    print(f"\n  Demo on NON-retrieval head KV0, group0, q_token0, Layer 24:")
    demo_w0 = all_attn[24, 0, 0, 0].numpy()
    print(f"  Original: top-5 = {np.sort(demo_w0)[::-1][:5]}")
    print(f"  Original: #above uniform = {(demo_w0 > 1.0/ctx_len).sum()}")

    for config in [(2.0, 1), (2.0, 2), (2.0, 3), (3.0, 1), (5.0, 1)]:
        alpha, n_iter = config
        sharp = iterative_power_sharpen(demo_w0, n_iter, alpha)
        above = (sharp > 1.0/ctx_len).sum()
        top5 = np.sort(sharp)[::-1][:5]
        ndl_above = sum(1 for p in needle_pos if sharp[p] > 1.0/ctx_len)
        print(f"  α={alpha:.0f}, iter={n_iter}: top-5 = [{', '.join(f'{v:.4f}' for v in top5)}], "
              f"#above = {above}, ndl = {ndl_above}")

    # ═══════════════════════════════════════════
    # Analysis: different configs, different layers
    # ═══════════════════════════════════════════
    print(f"\n{'='*100}")
    print("Token count analysis per layer")
    print(f"{'='*100}")

    configs = [(2.0, 1), (2.0, 2), (2.0, 3), (3.0, 1), (3.0, 2), (5.0, 1), (10.0, 1)]
    ...
    for li in [20, 24, 28, 32]:
        ...
        keep_sets = select_tokens_power_sharpen(
            layer_attn, 0, 1.0, n_kv_heads, n_groups, n_q, ctx_len)
        sizes = [len(s) for s in keep_sets]
        mean_size = np.mean(sizes)
        cr = 1.0 - mean_size / ctx_len
        mean_needle = np.mean([len(needle_set & keep_sets[hi]) / len(needle_set) * 100 for hi in range(n_kv_heads)])
        print(f"  {'original':>12} ", end="")
        for s in sizes:
            print(f"{s:>8}", end="")
        print(f" {mean_size:>7.0f} {cr:>7.1%} {mean_needle:>7.1f}%")

        for alpha, n_iter in configs:
            keep_sets = select_tokens_power_sharpen(
                layer_attn, n_iter, alpha, n_kv_heads, n_groups, n_q, ctx_len)
            sizes = [len(s) for s in keep_sets]
            mean_size = np.mean(sizes)
            cr = 1.0 - mean_size / ctx_len
            mean_needle = np.mean([len(needle_set & keep_sets[hi]) / len(needle_set) * 100 for hi in range(n_kv_heads)])
            label = f"α{alpha:.0f}×{n_iter}"
    ...
    # ═══════════════════════════════════════════
    # Eviction test: per-layer power sharpening
    # ═══════════════════════════════════════════
    print(f"\n{'='*100}")
    print("Eviction test: per-layer power sharpening")
    print(f"{'='*100}")
    ...
    for alpha, n_iter in configs:
        ...
        label = f"α{alpha:.0f}×{n_iter}"
        print(f"  {label:>8}: CR={cr:.1%}, Answer={answer[:55]}... {'✓' if correct else '✗'}")
        del cache
        torch.cuda.empty_cache()

    # ═══════════════════════════════════════════
    # Eviction test: retrieval layers attention + power sharpening for ALL layers
    # ═══════════════════════════════════════════
    print(f"\n{'='*100}")
    print("Eviction test: retrieval layers (L24,28,32) + power sharpen for ALL layers")
    print(f"{'='*100}")

    retrieval_layers = [l for l in [24, 28, 32] if l < n_layers]
    agg_attn = all_attn[retrieval_layers].mean(dim=0)

    for alpha, n_iter in [(2.0, 1), (2.0, 2), (3.0, 1), (5.0, 1)]:
        keep_sets = select_tokens_power_sharpen(
            agg_attn, n_iter, alpha, n_kv_heads, n_groups, n_q, ctx_len)
        ...
        correct = target_value in answer
        label = f"α{alpha:.0f}×{n_iter}"
        print(f"  {label:>8}: CR={cr:.1%}, keep~{mean_keep:.0f}/head, "
              f"Answer={answer[:50]}... {'✓' if correct else '✗'}")
        del cache
        torch.cuda.empty_cache()

    # ═══════════════════════════════════════════
    # Eviction test: union(per-layer, global) power sharpening
    # ═══════════════════════════════════════════
    print(f"\n{'='*100}")
    print("Eviction test: union(per-layer, global) power sharpening")
    print(f"{'='*100}")

    for alpha, n_iter in [(2.0, 1), (2.0, 2), (3.0, 1), (5.0, 1)]:
        global_keep = select_tokens_power_sharpen(
            agg_attn, n_iter, alpha, n_kv_heads, n_groups, n_q, ctx_len)
        ...
        for li in range(n_layers):
            layer_attn = all_attn[li]
            local_keep = select_tokens_power_sharpen(
                layer_attn, n_iter, alpha, n_kv_heads, n_groups, n_q, ctx_len)
